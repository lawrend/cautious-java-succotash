...
public class NodeExample {
	...
	public static void main(String[] args) {
		...
		Node<Integer> firstNode = n4; 
		
		
//		System.out.println("Line 1: current is now " + current.data);
		printNums(firstNode);
		
		Node<Integer> current = firstNode;
		System.out.println("Line 2: current is now " + current.data);
		printNums(firstNode);
		
		current = current.next;
		System.out.println("Line 3: current is now " + current.data);
		printNums(firstNode);
		
		firstNode = firstNode.next;
		System.out.println("Line 4: current is now " + current.data);
		printNums(firstNode);
		
		current.next.data = 36;
		System.out.println("Line 5: current is now " + current.data);
		printNums(firstNode);
		
		current.next = current.next.next.next;
		
		System.out.println("Line 6: current is now " + current.data);
		firstNode.data = 17;
		
		printNums(firstNode);
		System.out.println("Line 7: current is now " + current.data);
		current = current.next;
		
		printNums(firstNode);
		System.out.println("Line 8: current is now " + current.data);
		firstNode.next = current.next;
		
		printNums(firstNode);
		System.out.println("Line 9: current is now " + current.data);
		...
		Node c = new Node(2, b);
		Node firstOne = c;
		Node currentNode = firstOne;
		currentNode = currentNode.next;
		currentNode = currentNode.next;
		System.out.println("answer is " + currentNode.next.data);	
...
//		printRoster(head); // print Barb 
	    ...
	    public void method(Node firstGuy) {
			   while(firstGuy != null) {
			      System.out.println(firstGuy.data);
			      firstGuy = firstGuy.next;
			   }
			} 
	
		public static void printNums(Node<Integer> firstNode) {
		Node<Integer> current = firstNode;
		
		while(current!=null) {
			Integer currentNum = current.data;
			//if(currentStudent.getStudentID().startsWith("A")) { 
			// an example of a conditional to only process some elements 
			
			System.out.println("data = " + currentNum );
			current = current.next;
		}
