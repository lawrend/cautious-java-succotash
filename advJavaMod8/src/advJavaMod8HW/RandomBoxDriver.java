...
public class RandomBoxDriver {
	...
	public static <T> List<T> pickMultipleUniqueWinners(RandomBox<T> obj, int numberOfWinners) {
		List<T> list = new ArrayList<T>();
		
		if(numberOfWinners > obj.size()) {
			numberOfWinners = obj.size();
		}
		
		for(int i = 0; i < numberOfWinners; i++) {
			list.add(obj.drawWinner());
		}
		return list;
	}
