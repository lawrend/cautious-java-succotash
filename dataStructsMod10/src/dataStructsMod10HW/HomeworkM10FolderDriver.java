...
public class HomeworkM10FolderDriver {
	...
	public static void printFolderContentsWithStack(Folder topFolder) {
		...
		while(!folderStack.isEmpty()) {
			if(!folderStack.peek().getFolderList().isEmpty()) {
                for(Folder folder : folderStack.peek().getFolderList()) {
                	 System.out.println(folder.getName());
				     folderStack.push(folder);
				     if(folder.getFolderList().isEmpty()) {
				        for(File file : folderStack.peek().getFileList()) {
